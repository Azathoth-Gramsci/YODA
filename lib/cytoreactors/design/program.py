...
class Program:
    def __init__(self, user, campaign, short_name, description, reactor_id, preculture, media, creation_date_shift=None, blank=None):
        self.user = user
    ...
    def load(self, ignore_new=False):
        ...
        for k in ['user', 'campaign', 'short_name', 'description', 'reactor_id', 'creation_date']:
...
class TurbidostatProgram(Program):
    def __init__(self, user, campaign, short_name, description, reactor_id, preculture, media, OD_setpoint, creation_date_shift=None, blank=None):
        self.base_dilution_duration = 3.
        self.est_flow_rate_uL_per_s = 250.
        self.OD_setpoint = OD_setpoint
        super().__init__(user, campaign, short_name, description, reactor_id, preculture, media, creation_date_shift, blank)
    def give_program_parameters(self):
...
class ChemostatProgram(Program):
    def __init__(self, user, campaign, short_name, description, reactor_id, preculture, media, dilution_duration_s=1.0, creation_date_shift=None, blank=None):
        self.dilution_duration_s = dilution_duration_s
        self.est_flow_rate_uL_per_s = 250.
        super().__init__(user, campaign, short_name, description, reactor_id, preculture, media, creation_date_shift, blank)
    def give_program_parameters(self):
        return {}
    def compute_dilution_duration(self):
        if (self.blank is not None) and self.has_OD_measurements():
            self.dilutions['time_s'].append(time())
            self.dilutions['duration_s'].append(self.dilution_duration_s)
            self.dilutions['est_flow_rate_uL_per_s'].append(self._manager.input_pump_flow_rate_mL_min(self.reactor_id)/60.*1000.)
            return self.dilutions['duration_s'][-1]
        else:
            return None
...
class GrowDiluteProgram(TurbidostatProgram):
    def __init__(self, user, campaign, short_name, description, reactor_id, preculture, media, OD_low, OD_high, creation_date_shift=None, blank=None):
        if OD_low >= OD_high:
            raise Exception('OD_low >= OD_high !!!!! Does not make sense.')
        self.est_flow_rate_uL_per_s = 250.
        self.OD_low = OD_low
        self.OD_high = OD_high
        super().__init__(user, campaign, short_name, description, reactor_id, preculture, media, OD_high, creation_date_shift, blank)
