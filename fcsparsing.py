...
MODIFIED BY FRANCOIS BERTAUX - INSTITUT PASTEUR FOR COMPATIBILITY WITH GUAVA CYTOMETER
"""
...
class FCSParser(object):
    ...
    def load_file(self, file_handle, data_set=0, read_data=True):
        ...
        while data_segments <= data_set:
            ...
            #print(self.annotation['$NEXTDATA'])
            if '$NEXTDATA' in self.annotation:
                ...
                if nextdata_offset == 0 and data_segments < data_set:
                    raise Exception("File does not contain the number of data sets.")
                    break
            else:
                if data_segments != 0:
                    raise Exception('File does not contain $NEXTDATA information.')
                break
        if read_data:
    ...
    def read_header(self, file_handle, nextdata_offset=0):
        ...
        for field in ('text start', 'text end', 'data start', 'data end', 'analysis start',
                      'analysis end'):
            #print("trying to read {}".format(field))
            s = file_handle.read(8)
            try:
                field_value = int(s)
                #print("result = {}".format(field_value))
            except ValueError:
                field_value = 0
        ...
        if header['text end'] == header['data start']:
            raise Exception("dont want this for now with guava")
        ...
        self._data_end = header['data end']
